...
UniValue transactionReceiptToJSON(const YuPostTransactionReceipt& txRec)
{
    UniValue result(UniValue::VOBJ);
...
UniValue CallToContract(const UniValue& params)
{
    ...
    if(params.size() >= 3){
        CTxDestination yupostprojectSenderAddress = DecodeDestination(params[2].get_str());
        if (IsValidDestination(yupostprojectSenderAddress)) {
            const PKHash *keyid = boost::get<PKHash>(&yupostprojectSenderAddress);
    ...
    for(size_t i = 0; i < list.size(); i++)
    {
        ...
        for(size_t i = 0; i < listLog.size(); i++)
        {
            ...
            if(numTopics > 1)
            {
                ...
                ToYuPostAddress(tokenEvent.sender, tokenEvent.sender);
            }
            if(numTopics > 2)
            {
                tokenEvent.receiver = topicsList[2].get_str().substr(24);
                ToYuPostAddress(tokenEvent.receiver, tokenEvent.receiver);
            }
