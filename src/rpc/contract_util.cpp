...
UniValue CallToContract(const UniValue& params)
{
    ...
    if(params.size() >= 3){
        CTxDestination yupostSenderAddress = DecodeDestination(params[2].get_str());
        if (IsValidDestination(yupostSenderAddress)) {
            const PKHash *keyid = boost::get<PKHash>(&yupostSenderAddress);
