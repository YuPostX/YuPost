...
UniValue blockheaderToJSON(const CBlockIndex* tip, const CBlockIndex* blockindex)
{
    ...
    result.pushKV("hashStateRoot", blockindex->hashStateRoot.GetHex()); // yupostproject
    result.pushKV("hashUTXORoot", blockindex->hashUTXORoot.GetHex()); // yupostproject

    if(blockindex->IsProofOfStake()){
        result.pushKV("prevoutStakeHash", blockindex->prevoutStake.hash.GetHex()); // yupostproject
        result.pushKV("prevoutStakeVoutN", (int64_t)blockindex->prevoutStake.n); // yupostproject
    }
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* tip, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.pushKV("hashStateRoot", block.hashStateRoot.GetHex()); // yupostproject
    result.pushKV("hashUTXORoot", block.hashUTXORoot.GetHex()); // yupostproject
...
static std::vector<RPCResult> MempoolEntryDescription() { return {
    ...
    RPCResult{RPCResult::Type::NUM, "size", "(DEPRECATED) same as vsize. Only returned if yupostprojectd is started with -deprecatedrpc=size\n"
...
UniValue getdelegationinfoforaddress(const JSONRPCRequest& request)
{
            RPCHelpMan{"getdelegationinfoforaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The yupostproject address string"},
                },
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
    ...
    YuPostDelegation yupostprojectDelegation;
    Delegation delegation;
    uint160 address = uint160(*pkhash);
    if(!yupostprojectDelegation.GetDelegation(address, delegation)) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Failed to get delegation");
    }
    bool verified = yupostprojectDelegation.VerifyDelegation(address, delegation);
...
UniValue getdelegationsforstaker(const JSONRPCRequest& request)
{
            RPCHelpMan{"getdelegationsforstaker",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The yupostproject address string for staker"},
                },
               RPCResult{
            RPCResult::Type::ARR, "", "",
                {
    ...
    if(!yupostprojectDelegation.FilterDelegationEvents(events, filter)) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Failed to get delegations for staker");
    }
    std::map<uint160, Delegation> delegations = yupostprojectDelegation.DelegationsFromEvents(events);
                        ...
                        {RPCResult::Type::OBJ, "scriptPubKey", "",
                            {
                                ...
                                {RPCResult::Type::ARR, "addresses", "array of yupostproject addresses",
                                    {{RPCResult::Type::STR, "address", "yupostproject address"}}},
                            }},
...
static UniValue qrc20balanceof(const JSONRPCRequest& request)
{
            RPCHelpMan{"qrc20balanceof",
                ...
                {
                    {"contractaddress", RPCArg::Type::STR_HEX, RPCArg::Optional::NO, "The contract address"},
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO,  "The yupostproject address to check token balance"},
                },
                RPCResult{
...
static UniValue qrc20allowance(const JSONRPCRequest& request)
{
            RPCHelpMan{"qrc20allowance",
                ...
                {
                    {"contractaddress", RPCArg::Type::STR_HEX, RPCArg::Optional::NO, "The contract address"},
                    {"addressFrom", RPCArg::Type::STR, RPCArg::Optional::NO,  "The yupostproject address of the account owning tokens"},
                    {"addressTo", RPCArg::Type::STR, RPCArg::Optional::NO,  "The yupostproject address of the account able to transfer the tokens"},
                },
                RPCResult{
...
static UniValue qrc20listtransactions(const JSONRPCRequest& request)
{
            RPCHelpMan{"qrc20listtransactions",
                ...
                {
                    {"contractaddress", RPCArg::Type::STR_HEX, RPCArg::Optional::NO, "The contract address."},
                    {"addresss", RPCArg::Type::STR, RPCArg::Optional::NO,  "The yupostproject address to get history for."},
            ...
            RPCResult::Type::ARR, "", "",
                {
                    {RPCResult::Type::OBJ, "", "",
                        {
                            {RPCResult::Type::STR, "receiver", "The receiver yupostproject address"},
                            {RPCResult::Type::STR, "sender", "The sender yupostproject address"},
