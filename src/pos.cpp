...
YuPostDelegation& GetYuPostDelegation()
{
    static YuPostDelegation yupostprojectDelegation;
    return yupostprojectDelegation;
}
...
bool CheckProofOfStake(CBlockIndex* pindexPrev, BlockValidationState& state, const CTransaction& tx, unsigned int nBits, uint32_t nTimeBlock, const std::vector<unsigned char>& vchPoD,  const COutPoint& headerPrevout, uint256& hashProofOfStake, uint256& targetProofOfStake, CCoinsViewCache& view)
{
    ...
    if (nHeight >= nOfflineStakeHeight && !Params().GetConsensus().delegationsAddress.IsNull())
    {
        ...
        YuPostDelegation& yupostprojectDelegation = GetYuPostDelegation();
        if(!yupostprojectDelegation.ExistDelegationContract())
        ...
        bool verifiedDelegation = yupostprojectDelegation.VerifyDelegation(address, delegation);
...
bool GetDelegationFeeFromContract(const uint160& address, uint8_t& fee)
{
    Delegation delegation;
    YuPostDelegation& yupostprojectDelegation = GetYuPostDelegation();
    bool ret = yupostprojectDelegation.GetDelegation(address, delegation);
    if(ret) ret &= yupostprojectDelegation.VerifyDelegation(address, delegation);
    if(ret)
    {
        fee = delegation.fee;
    }
