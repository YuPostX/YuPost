...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setYuPostSchedule(yupostprojectDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = yupostprojectDGP.getBlockSize(nHeight);
    minGasPrice = yupostprojectDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = yupostprojectDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    YuPostTxConverter convert(iter->GetTx(), NULL, &pblock->vtx, contractflags);

    ExtractYuPostTX resultConverter;
    if(!convert.extractionYuPostTransactions(resultConverter)){
    ...
    for(YuPostTransaction yupostprojectTransaction : yupostprojectTransactions){
        txGas += yupostprojectTransaction.gas();
        if(txGas > txGasLimit) {
        ...
        if(bceResult.usedGas + yupostprojectTransaction.gas() > softBlockGasLimit){
        ...
        if(yupostprojectTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, yupostprojectTransactions, hardBlockGasLimit, ::ChainActive().Tip());
...
public:
    ...
    void Update(int32_t nHeight)
    {
        ...
        if(nHeight <= checkpointSpan)
        {
            ...
            yupostprojectDelegations.FilterDelegationEvents(events, *this);
            delegations_staker = yupostprojectDelegations.DelegationsFromEvents(events);
        }
        else
        {
            ...
            {
                std::vector<DelegationEvent> events;
                yupostprojectDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                yupostprojectDelegations.UpdateDelegationsFromEvents(events, cacheDelegationsStaker);
                cacheHeight = cpsHeight;
            }

            // Update the wallet delegations
            std::vector<DelegationEvent> events;
            yupostprojectDelegations.FilterDelegationEvents(events, *this, cacheHeight + 1);
            delegations_staker = cacheDelegationsStaker;
            yupostprojectDelegations.UpdateDelegationsFromEvents(events, delegations_staker);
        }
...
private:
    CWallet *pwallet;
    YuPostDelegation yupostprojectDelegations;
    int32_t cacheHeight;
    ...
    void Update(interfaces::Chain::Lock& locked_chain, int32_t nHeight)
    {
        if(fLogEvents)
        {
            ...
            if(nHeight <= checkpointSpan)
            {
                ...
                std::vector<DelegationEvent> events;
                yupostprojectDelegations.FilterDelegationEvents(events, *this);
                pwallet->m_my_delegations = yupostprojectDelegations.DelegationsFromEvents(events);
            }
            else
            {
                // Update the cached delegations for the staker, older then the sync checkpoint (500 blocks)
                ...
                if(cacheHeight < cpsHeight)
                {
                    std::vector<DelegationEvent> events;
                    yupostprojectDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                    yupostprojectDelegations.UpdateDelegationsFromEvents(events, cacheMyDelegations);
                    cacheHeight = cpsHeight;
                }
                ...
                yupostprojectDelegations.UpdateDelegationsFromEvents(events, pwallet->m_my_delegations);
            }
        }
        else
        {
            ...
            if(cacheHeight != nHeight)
            {
                ...
                for(auto item: mapAddress)
                {
                    ...
                    if(yupostprojectDelegations.GetDelegation(address, delegation) && YuPostDelegation::VerifyDelegation(address, delegation))
                    {
...
public:
...

    {
        ...
        std::string threadName = "yupostprojectstake";
...
void StakeYuPosts(bool fStake, CWallet *pwallet, CConnman* connman, boost::thread_group*& stakeThread)
{
