...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setYuPostSchedule(yupostDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = yupostDGP.getBlockSize(nHeight);
    minGasPrice = yupostDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = yupostDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    for(YuPostTransaction yupostTransaction : yupostTransactions){
        ...
        if(bceResult.usedGas + yupostTransaction.gas() > softBlockGasLimit){
        ...
        if(yupostTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, yupostTransactions, hardBlockGasLimit, ::ChainActive().Tip());
...
public:
    ...
    void Update(int32_t nHeight)
    {
        ...
        if(nHeight <= checkpointSpan)
        {
            ...
            yupostDelegations.FilterDelegationEvents(events, *this);
            delegations_staker = yupostDelegations.DelegationsFromEvents(events);
        }
        else
        {
            ...
            {
                std::vector<DelegationEvent> events;
                yupostDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                yupostDelegations.UpdateDelegationsFromEvents(events, cacheDelegationsStaker);
                cacheHeight = cpsHeight;
            }

            // Update the wallet delegations
            std::vector<DelegationEvent> events;
            yupostDelegations.FilterDelegationEvents(events, *this, cacheHeight + 1);
            delegations_staker = cacheDelegationsStaker;
            yupostDelegations.UpdateDelegationsFromEvents(events, delegations_staker);
        }
...
private:
    CWallet *pwallet;
    YuPostDelegation yupostDelegations;
    int32_t cacheHeight;
    ...
    void Update(interfaces::Chain::Lock& locked_chain, int32_t nHeight)
    {
        if(fLogEvents)
        {
            ...
            if(nHeight <= checkpointSpan)
            {
                ...
                std::vector<DelegationEvent> events;
                yupostDelegations.FilterDelegationEvents(events, *this);
                pwallet->m_my_delegations = yupostDelegations.DelegationsFromEvents(events);
            }
            else
            {
                // Update the cached delegations for the staker, older then the sync checkpoint (500 blocks)
                ...
                if(cacheHeight < cpsHeight)
                {
                    std::vector<DelegationEvent> events;
                    yupostDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                    yupostDelegations.UpdateDelegationsFromEvents(events, cacheMyDelegations);
                    cacheHeight = cpsHeight;
                }
                ...
                yupostDelegations.UpdateDelegationsFromEvents(events, pwallet->m_my_delegations);
            }
        }
        else
        {
            ...
            if(cacheHeight != nHeight)
            {
                ...
                for(auto item: mapAddress)
                {
                    ...
                    if(yupostDelegations.GetDelegation(address, delegation) && YuPostDelegation::VerifyDelegation(address, delegation))
                    {
...
public:
...

    {
        ...
        std::string threadName = "yupoststake";
