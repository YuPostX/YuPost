...
public:
    ...
    bool isUnspentAddress(const std::string &yupostAddress) override
    {
        auto locked_chain = m_wallet->chain().lock();
        LOCK(m_wallet->cs_wallet);
        ...
        for (const COutput& out : vecOutputs)
        {
            ...
            if(fValidAddress && EncodeDestination(address) == yupostAddress && out.tx->tx->vout[out.i].nValue)
            {
                return true;
            }
        }
    ...
    DelegationInfo getDelegationContract(const std::string &sHash, bool& validated, bool& contractRet) override
    {
        ...
        if (mi != m_wallet->mapDelegation.end()) {
            ...
            if(keyID)
            {
                ...
                contractRet = m_yupostDelegation.ExistDelegationContract() ? m_yupostDelegation.GetDelegation(address, delegation) : false;
                if(contractRet)
                {
                    validated = m_yupostDelegation.VerifyDelegation(address, delegation);
    ...
    DelegationDetails getDelegationDetails(const std::string &sAddress) override
    {
        ...
        {
            uint160 address(*keyID);
            details.c_contract_return = m_yupostDelegation.ExistDelegationContract() ? m_yupostDelegation.GetDelegation(address, delegation) : false;
            if(details.c_contract_return)
            {
                details.c_entry_exist = m_yupostDelegation.VerifyDelegation(address, delegation);
    ...
    YuPostDelegation m_yupostDelegation;
};
