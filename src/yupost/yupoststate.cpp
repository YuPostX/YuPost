...
YuPostState::YuPostState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute YuPostState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, YuPostTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createYuPostAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    if(voutLimit){
        ...
        return ResultExecute{ex, YuPostTransactionReceipt(oldStateRoot, oldUTXORoot, gas, e.logs()), refund.vout.empty() ? CTransaction() : CTransaction(refund)};
    }else{
        return ResultExecute{res, YuPostTransactionReceipt(rootHash(), rootHashUTXO(), startGasUsed + e.gasUsed(), e.logs()), tx ? *tx : CTransaction()};
    }
}

std::unordered_map<dev::Address, Vin> YuPostState::vins() const // temp
{
...
void YuPostState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* YuPostState::vin(dev::Address const& _a) const
{
    return const_cast<YuPostState*>(this)->vin(_a);
}

Vin* YuPostState::vin(dev::Address const& _addr)
{
...
// void YuPostState::commit(CommitBehaviour _commitBehaviour)
// {
...
void YuPostState::kill(dev::Address _addr)
{
...
void YuPostState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
void YuPostState::deleteAccounts(std::set<dev::Address>& addrs){
...
void YuPostState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void YuPostState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
    clog(dev::VerbosityWarning, "exec") << "VM exception:" << ss.str();
}

void YuPostState::validateTransfersWithChangeLog(){
...
void YuPostState::deployDelegationsContract(){
    dev::Address delegationsAddress = uintToh160(Params().GetConsensus().delegationsAddress);
    if(!YuPostState::addressInUse(delegationsAddress)){
        YuPostState::createContract(delegationsAddress);
        YuPostState::setCode(delegationsAddress, bytes{fromHex(DELEGATIONS_CONTRACT_CODE)}, YuPostState::version(delegationsAddress));
        commit(CommitBehaviour::RemoveEmptyAccounts);
        db().commit();
    }
}
