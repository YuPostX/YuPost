...
void YuPostDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool YuPostDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
dev::eth::EVMSchedule YuPostDGP::getGasSchedule(int blockHeight){
    clear();
...
uint64_t YuPostDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t YuPostDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t YuPostDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t YuPostDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool YuPostDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void YuPostDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void YuPostDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void YuPostDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void YuPostDGP::createParamsInstance(){
...
dev::Address YuPostDGP::getAddressForBlock(unsigned int blockHeight){
...
void YuPostDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void YuPostDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void YuPostDGP::parseStorageOneUint64(uint64_t& value){
...
void YuPostDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule YuPostDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
