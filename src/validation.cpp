...
#if defined(NDEBUG)
# error "YuPost cannot be compiled without assertions."
#endif
...
std::unique_ptr<YuPostState> globalState;
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if(tx.HasCreateOrCall()){
        ...
        YuPostTxConverter converter(tx, NULL, NULL, contractflags);
        ExtractYuPostTX resultConverter;
...
bool MemPoolAccept::Finalize(ATMPArgs& args, Workspace& ws)
{
    ...
    for (CTxMemPool::txiter it : allConflicting)
    {
        LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s YPO additional fees, %d delta bytes\n",
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit, CAmount nAmount){
    ...
    YuPostTransaction callTransaction;
    if(addrContract == dev::Address())
    {
        callTransaction = YuPostTransaction(nAmount, 1, dev::u256(gasLimit), opcode, nonce);
    }
    else
    {
        callTransaction = YuPostTransaction(nAmount, 1, dev::u256(gasLimit), addrContract, opcode, nonce);
    }
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<YuPostTransaction>(1, callTransaction), blockGasLimit, pblockindex);
...
bool ByteCodeExec::performByteCode(dev::eth::Permanence type){
    for(YuPostTransaction& tx : txs){
        ...
        if(!tx.isCreation() && !globalState->addressInUse(tx.receiveAddress())){
            ...
            result.push_back(ResultExecute{execRes, YuPostTransactionReceipt(dev::h256(), dev::h256(), dev::u256(), dev::eth::LogEntries()), CTransaction()});
            continue;
        }
...
bool YuPostTxConverter::extractionYuPostTransactions(ExtractYuPostTX& yupostprojecttx){
    ...
    std::vector<YuPostTransaction> resultTX;
...
bool YuPostTxConverter::receiveStack(const CScript& scriptPubKey){
    sender = false;
...
bool YuPostTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
YuPostTransaction YuPostTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    YuPostTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = YuPostTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = YuPostTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
...
size_t YuPostTxConverter::correctedStackSize(size_t size){
...
///////////////////////////////////////////////////////////////////////////////////////// yupostproject
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            YuPostTxConverter convert(tx, &view, &block.vtx, contractflags);

            ExtractYuPostTX resultConvertYuPostTX;
            if(!convert.extractionYuPostTransactions(resultConvertYuPostTX)){
            ...
            if(!CheckMinGasPrice(resultConvertYuPostTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertYuPostTX.first, blockGasLimit, pindex->pprev);
            ...
            for(YuPostTransaction& qtx : resultConvertYuPostTX.first){
            ...
            if (fLogEvents && !fJustCheck)
            {
                ...
                for(size_t k = 0; k < resultConvertYuPostTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{
                        ...
                        tx.GetHash(),
                        uint32_t(i),
                        resultConvertYuPostTX.first[k].from(),
                        resultConvertYuPostTX.first[k].to(),
                        ...
                        resultConvertYuPostTX.first[k].getNVout(),
...
bool SignBlockHWI(std::shared_ptr<CBlock> pblock, CWallet& wallet, std::vector<unsigned char>& vchSig)
{
    ...
    YuPostLedger &device = YuPostLedger::instance();
...
bool SignBlockLedger(std::shared_ptr<CBlock> pblock, CWallet& wallet, std::vector<unsigned char>& vchSig)
{
    ...
    {
        std::string errorMessage = YuPostLedger::instance().errorMessage();
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for YuPost PoS we need to sync this after every block to ensure txdb is populated for
...
CAmount GetTxGasFee(const CMutableTransaction& _tx)
{
    ...
    if(tx.HasCreateOrCall())
    {
        ...
        YuPostTxConverter convert(tx, NULL, NULL, contractflags);
