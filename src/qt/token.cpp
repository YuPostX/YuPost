...
Token::Token()
{
    ...
    lstMandatory.append(YuPostToken::paramAddress());
    lstMandatory.append(YuPostToken::paramDatahex());
    QStringList lstOptional;
    lstOptional.append(YuPostToken::paramSender());
    ...
    lstMandatory.append(YuPostToken::paramDatahex());
    lstOptional.clear();
    lstOptional.append(YuPostToken::paramAmount());
    lstOptional.append(YuPostToken::paramGasLimit());
    lstOptional.append(YuPostToken::paramGasPrice());
    lstOptional.append(YuPostToken::paramSender());
    lstOptional.append(YuPostToken::paramBroadcast());
    lstOptional.append(YuPostToken::paramChangeToSender());
    lstOptional.append(YuPostToken::paramPsbt());
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            if(numTopics > 1)
            {
                ...
                Token::ToYuPostAddress(tokenEvent.sender, tokenEvent.sender);
            }
            if(numTopics > 2)
            {
                tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
                Token::ToYuPostAddress(tokenEvent.receiver, tokenEvent.receiver);
            }
