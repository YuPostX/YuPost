    ...
    m_mode(mode)
{
    ...
    ui->labelAddress->setToolTip(tr("The yupostproject address to split utxos."));
...
void SplitUTXOPage::setModel(WalletModel *_model)
{
    ...
    if (bCreateUnsigned) {
        ...
        ui->splitCoinsButton->setToolTip(tr("Creates a Partially Signed YuPost Transaction (PSBT) for use with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));
    }
