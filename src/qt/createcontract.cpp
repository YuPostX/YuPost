    ...
    m_results(1)
{
    ...
    ui->labelSenderAddress->setToolTip(tr("The yupostproject address that will be used to create the contract."));
...
void CreateContract::setModel(WalletModel *_model)
{
    ...
    if (bCreateUnsigned) {
        ...
        ui->pushButtonCreateContract->setToolTip(tr("Creates a Partially Signed YuPost Transaction (PSBT) for use with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));
    }
}
...
void CreateContract::on_createContractClicked()
{
    if(isDataValid())
    {
        ...
        if (bCreateUnsigned) {
            ...
            questionString.append(tr("This will produce a Partially Signed YuPost Transaction (PSBT) which you can copy and then sign with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));
