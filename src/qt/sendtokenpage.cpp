...
void SendTokenPage::setModel(WalletModel *_model)
{
    ...
    if (bCreateUnsigned) {
        ...
        ui->confirmButton->setToolTip(tr("Creates a Partially Signed YuPost Transaction (PSBT) for use with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));
    }
}
