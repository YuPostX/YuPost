...
void RemoveDelegationPage::setModel(WalletModel *_model)
{
    ...
    if (bCreateUnsigned) {
        ...
        ui->removeDelegationButton->setToolTip(tr("Creates a Partially Signed YuPost Transaction (PSBT) for use with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));
    }
}
