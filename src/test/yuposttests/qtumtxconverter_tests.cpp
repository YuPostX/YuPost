...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    ExtractYuPostTX yupostTx;
    BOOST_CHECK(converter.extractionYuPostTransactions(yupostTx));
    std::vector<YuPostTransaction> result = yupostTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(yuposttxconverter_tests, TestingSetup)
