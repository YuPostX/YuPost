...
inline void initState(){
    ...
    const std::string dirYuPost = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<YuPostState>(new YuPostState(dev::u256(0), YuPostState::openDB(dirYuPost, hashDB, dev::WithExisting::Trust), dirYuPost + "/yupostprojectDB", dev::eth::BaseState::Empty));
...
inline dev::Address createYuPostAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline YuPostTransaction createYuPostTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    YuPostTransaction txEth;
    if(recipient == dev::Address()){
        txEth = YuPostTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = YuPostTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<YuPostTransaction> txs){
