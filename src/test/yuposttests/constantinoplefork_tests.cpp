...
//    initState();
    ...
    txs.push_back(createYuPostTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), hashTx, dev::Address()));
    txs.push_back(createYuPostTransaction(CODE[1], 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
    executeBC(txs);

    // Call upgrade to
    dev::Address proxy = createYuPostAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<YuPostTransaction> txsCall;
    txsCall.push_back(createYuPostTransaction(CODE[2], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    executeBC(txsCall);

    // Call mint
    std::vector<YuPostTransaction> txsMint;
    txsMint.push_back(createYuPostTransaction(CODE[3], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    auto result = executeBC(txsMint);
    BOOST_CHECK(result.first[0].execRes.excepted == dev::eth::TransactionException::None);

    // Call balance of
    std::vector<YuPostTransaction> txsbalance;
    txsbalance.push_back(createYuPostTransaction(ParseHex("70a082310000000000000000000000000101010101010101010101010101010101010101"), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
...
//    initState();
    ...
    // Create contract
    std::vector<YuPostTransaction> txs;
    txs.push_back(createYuPostTransaction(CODE[4], 0, GASLIMIT, dev::u256(1), hashTx, dev::Address()));
    executeBC(txs);

    // Call is it constantinople
    dev::Address proxy = createYuPostAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<YuPostTransaction> txIsItConstantinople;
    txIsItConstantinople.push_back(createYuPostTransaction(CODE[5], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
