...
void checkResult(bool isCreation, std::vector<YuPostTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    YuPostTxConverter converter(transaction, NULL);
    ExtractYuPostTX yupostprojectTx;
    BOOST_CHECK(converter.extractionYuPostTransactions(yupostprojectTx));
    std::vector<YuPostTransaction> result = yupostprojectTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(yupostprojecttxconverter_tests, TestingSetup)
