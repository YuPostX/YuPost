...
void createTestContractsAndBlocks(TestChain100Setup* testChain100Setup, const valtype& code1, const valtype& code2, const valtype& code3, dev::Address addr){
    ...
    txs.push_back(createYuPostTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, addr, 0));
    txs.push_back(createYuPostTransaction(code1, 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createYuPostTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, addr, 0));
    auto result = executeBC(txs);

    generateBlocks(50);
    txs.clear();
    txs.push_back(createYuPostTransaction(code2, 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createYuPostTransaction(code[4], 0, dev::u256(500000), dev::u256(1), ++hashTemp, addr, 0));
    result = executeBC(txs);

    generateBlocks(50);
    txs.clear();
    txs.push_back(createYuPostTransaction(code3, 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createYuPostTransaction(code[6], 0, dev::u256(500000), dev::u256(1), ++hashTemp, addr, 0));
    result = executeBC(txs);
}
...
BOOST_AUTO_TEST_CASE(gas_schedule_default_state_test1){
    initState();
    contractLoading();
    YuPostDGP yupostprojectDGP(globalState.get());
    dev::eth::EVMSchedule schedule = yupostprojectDGP.getGasSchedule(100);
...
BOOST_AUTO_TEST_CASE(gas_schedule_default_state_test2){
    ...
    contractLoading();
    YuPostDGP yupostprojectDGP(globalState.get());
    dev::eth::EVMSchedule schedule = yupostprojectDGP.getGasSchedule(0);
    BOOST_CHECK(compareEVMSchedule(schedule, dev::eth::EIP158Schedule));
}
...
BOOST_AUTO_TEST_CASE(gas_schedule_default_state_test3){
    ...
    dev::eth::EVMSchedule schedule = yupostprojectDGP.getGasSchedule(coinbaseMaturity + 900);
...
BOOST_AUTO_TEST_CASE(gas_schedule_one_paramsInstance_introductory_block_1_test1){
    ...
    dev::h256 hashTemp(hash);
    std::vector<YuPostTransaction> txs;
    txs.push_back(createYuPostTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, GasScheduleDGP, 0));
    txs.push_back(createYuPostTransaction(code[1], 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createYuPostTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, GasScheduleDGP, 0));
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(gas_schedule_one_paramsInstance_introductory_block_1_test2){
    ...
    YuPostDGP yupostprojectDGP(globalState.get());
    int coinbaseMaturity = Params().GetConsensus().CoinbaseMaturity(0);
    dev::eth::EVMSchedule schedule = yupostprojectDGP.getGasSchedule(coinbaseMaturity + 2); // After initializing the tests, the height of the chain 502
...
//    initState();
    ...
    for(size_t i = 0; i < sizeList; i++){
        dev::eth::EVMSchedule schedule = yupostprojectDGP.getGasSchedule(i);
...
BOOST_AUTO_TEST_CASE(block_size_default_state_test1){
    ...
    uint32_t blockSize = yupostprojectDGP.getBlockSize(nHeight);
...
BOOST_AUTO_TEST_CASE(block_size_default_state_test2){
    ...
    uint32_t nHeight = 0;
    uint32_t blocktimeDownscaleFactor = Params().GetConsensus().BlocktimeDownscaleFactor(nHeight);
    uint32_t blockSize = yupostprojectDGP.getBlockSize(0);
    BOOST_CHECK(blockSize == DEFAULT_BLOCK_SIZE_DGP / blocktimeDownscaleFactor);
}
...
BOOST_AUTO_TEST_CASE(block_size_one_paramsInstance_introductory_block_1_test1){
    ...
    dev::h256 hashTemp(hash);
    std::vector<YuPostTransaction> txs;
    txs.push_back(createYuPostTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, BlockSizeDGP, 0));
    txs.push_back(createYuPostTransaction(code[7], 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createYuPostTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, BlockSizeDGP, 0));
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(block_size_one_paramsInstance_introductory_block_1_test2){
    ...
    YuPostDGP yupostprojectDGP(globalState.get());
    int coinbaseMaturity = Params().GetConsensus().CoinbaseMaturity(0);
    uint32_t blockSize = yupostprojectDGP.getBlockSize(coinbaseMaturity + 2);
...
//    initState();
    ...
    for(size_t i = 0; i < sizeList; i++){
        ...
        uint32_t blockSize = yupostprojectDGP.getBlockSize(i);
...
BOOST_AUTO_TEST_CASE(min_gas_price_default_state_test1){
    ...
    contractLoading();
    YuPostDGP yupostprojectDGP(globalState.get());
    uint64_t minGasPrice = yupostprojectDGP.getMinGasPrice(100);
...
BOOST_AUTO_TEST_CASE(min_gas_price_default_state_test2){
    ...
    contractLoading();
    YuPostDGP yupostprojectDGP(globalState.get());
    uint64_t minGasPrice = yupostprojectDGP.getMinGasPrice(0);
    BOOST_CHECK(minGasPrice == DEFAULT_MIN_GAS_PRICE_DGP);
}
...
BOOST_AUTO_TEST_CASE(min_gas_price_one_paramsInstance_introductory_block_1_test1){
    ...
    dev::h256 hashTemp(hash);
    std::vector<YuPostTransaction> txs;
    txs.push_back(createYuPostTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, GasPriceDGP, 0));
    txs.push_back(createYuPostTransaction(code[10], 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createYuPostTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, GasPriceDGP, 0));
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(min_gas_price_one_paramsInstance_introductory_block_1_test2){
    ...
    YuPostDGP yupostprojectDGP(globalState.get());
    int coinbaseMaturity = Params().GetConsensus().CoinbaseMaturity(0);
    uint64_t minGasPrice = yupostprojectDGP.getMinGasPrice(coinbaseMaturity + 2);
...
//    initState();
    ...
    size_t sizeList = Params().GetConsensus().CoinbaseMaturity(0) + 800;
    for(size_t i = 0; i < sizeList; i++){
        uint64_t minGasPrice = yupostprojectDGP.getMinGasPrice(i);
        std::function<bool(const uint64_t&, const uint64_t&)> func = compareUint64;
