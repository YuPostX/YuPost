...
class YuPostTransactionReceipt: public dev::eth::TransactionReceipt {
public:
    YuPostTransactionReceipt(dev::h256 const& state_root, dev::h256 const& utxo_root, dev::u256 const& gas_used, dev::eth::LogEntries const& log) : dev::eth::TransactionReceipt(state_root, gas_used, log), m_utxoRoot(utxo_root) {}
...
struct ResultExecute{
    ...
    YuPostTransactionReceipt txRec;
    CTransaction tx;
};

namespace yupostproject{
    template <class DB>
...
class YuPostState : public dev::eth::State {
    
public:

    YuPostState();

    YuPostState(dev::u256 const& _accountStartNonce, dev::OverlayDB const& _db, const std::string& _path, dev::eth::BaseState _bs = dev::eth::BaseState::PreExisting);

    ResultExecute execute(dev::eth::EnvInfo const& _envInfo, dev::eth::SealEngineFace const& _sealEngine, YuPostTransaction const& _t, dev::eth::Permanence _p = dev::eth::Permanence::Committed, dev::eth::OnOpFunc const& _onOp = OnOpFunc());
    ...
    static const dev::Address createYuPostAddress(dev::h256 hashTx, uint32_t voutNumber){
...
struct TemporaryState{
    std::unique_ptr<YuPostState>& globalStateRef;
    dev::h256 oldHashStateRoot;
    dev::h256 oldHashUTXORoot;

    TemporaryState(std::unique_ptr<YuPostState>& _globalStateRef) : 
...
public:
    ...
    CondensingTX(YuPostState* _state, const std::vector<TransferInfo>& _transfers, const YuPostTransaction& _transaction, std::set<dev::Address> _deleteAddresses = std::set<dev::Address>()) : transfers(_transfers), deleteAddresses(_deleteAddresses), transaction(_transaction), state(_state){}
...
private:
    ...
    const YuPostTransaction& transaction;
