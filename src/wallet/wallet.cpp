...
void CWallet::StakeYuPosts(bool fStake, CConnman* connman)
{
    ::StakeYuPosts(fStake, this, connman, stakeThread);
}
...
void CWallet::StartStake(CConnman *connman)
{
    if(IsWalletFlagSet(WALLET_FLAG_DISABLE_PRIVATE_KEYS))
    {
        m_enabled_staking = !m_ledger_id.empty() && YuPostLedger::instance().toolExists();
    }
    else
    {
