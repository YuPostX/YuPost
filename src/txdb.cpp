    ...
    while (pcursor->Valid()) {
        ...
        if (pcursor->GetKey(key) && key.first == DB_BLOCK_INDEX) {
            ...
            if (pcursor->GetValue(diskindex)) {
                ...
                pindexNew->hashStateRoot  = diskindex.hashStateRoot; // yupost
                pindexNew->hashUTXORoot   = diskindex.hashUTXORoot; // yupost
                pindexNew->nStakeModifier = diskindex.nStakeModifier;
                pindexNew->prevoutStake   = diskindex.prevoutStake;
                pindexNew->vchBlockSigDlgt    = diskindex.vchBlockSigDlgt; // yupost
