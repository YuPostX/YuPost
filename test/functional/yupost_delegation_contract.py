...
class YuPostSimpleDelegationContractTest(BitcoinTestFramework):
    ...
    """
        - Check that it is not possible to use an input with less value than 100 yupost as the first input to the coinstake.
        - Check that 100 yupost is just enough for the first input to the coinstake.
    """
    def nas_staker_must_be_100_yupost_test(self):
        use_pos_reward = True if self.staker.getblockcount() > 5000 else False
    ...
    def run_test(self):
        ...
        print("nas_staker_must_be_100_yupost_test")
        self.nas_staker_must_be_100_yupost_test()
        self.sync_all()
