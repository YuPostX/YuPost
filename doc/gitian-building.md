...
- [Building YuPost Core](#building-yupost-core)
...
Clone the git repositories for yupost and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/yupost/yupost --recursive
git clone https://github.com/yupost/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/yupost/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the yupost repository under 'Fetch and create inputs' to install sources which require
...
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the yupost repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/yupost/.git/
...
For example:
```bash
URL=https://github.com/laanwj/yupost.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit yupost=${COMMIT} --url yupost=${URL} ../yupost/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit yupost=${COMMIT} --url yupost=${URL} ../yupost/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit yupost=${COMMIT} --url yupost=${URL} ../yupost/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-yupost:
./bin/gbuild --commit yupost=${COMMIT},cpp-eth-yupost=${ETHCOMMIT} --url yupost=${URL},cpp-eth-yupost=${ETHURL} ../yupost/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the yupost git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../yupost/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/yupost/yupost-detached-sigs.git

BTCPATH=/some/root/path/yupost
SIGPATH=/some/root/path/yupost-detached-sigs

./bin/gbuild --url yupost=${BTCPATH},signature=${SIGPATH} ../yupost/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/yupost-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/yupost-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/yupost-osx-build.assert
```
...
[yupost/gitian.sigs](https://github.com/yupost/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@yupost.org) and he will commit them.
