Gitian building
================
*Setup instructions for a Gitian build of YuPost Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the YuPost
...
- [Building YuPost Core](#building-yupostproject-core)
...
Clone the git repositories for yupostproject and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/yupostproject/yupostproject --recursive
git clone https://github.com/yupostproject/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/yupostproject/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the yupostproject repository under 'Fetch and create inputs' to install sources which require
...
To build YuPost Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the yupostproject repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/yupostproject/.git/
...
For example:
```bash
URL=https://github.com/laanwj/yupostproject.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit yupostproject=${COMMIT} --url yupostproject=${URL} ../yupostproject/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit yupostproject=${COMMIT} --url yupostproject=${URL} ../yupostproject/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit yupostproject=${COMMIT} --url yupostproject=${URL} ../yupostproject/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-yupostproject:
./bin/gbuild --commit yupostproject=${COMMIT},cpp-eth-yupostproject=${ETHCOMMIT} --url yupostproject=${URL},cpp-eth-yupostproject=${ETHURL} ../yupostproject/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the yupostproject git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../yupostproject/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/yupostproject/yupostproject-detached-sigs.git

BTCPATH=/some/root/path/yupostproject
SIGPATH=/some/root/path/yupostproject-detached-sigs

./bin/gbuild --url yupostproject=${BTCPATH},signature=${SIGPATH} ../yupostproject/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/yupostproject-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/yupostproject-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/yupostproject-osx-build.assert
```
...
[yupostproject/gitian.sigs](https://github.com/yupostproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@yupostproject.org) and he will commit them.
