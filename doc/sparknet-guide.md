...
The smart contract interface in YuPost still requires some technical knowledge. The GUI is not completed yet, so all smart contract interation must happen either using `yupostproject-cli` at the command line, or in the debug window of `yupostproject-qt`. 
...
(note, if using the debug window in the YuPost Qt application, don't include `./yupostproject-cli` in the commands)
...
Now wait for your contract to be included in a block. You should be able to confirm it made it into a block by using:
    ...
    ./yupostproject-cli getaccountinfo $CONTRACT
...
Now, because we are not changing state, we use `callcontract`:
    ...
    ./yupostproject-cli callcontract $CONTRACT 3450bd6a
...
Note we use --lenient so that we do not need to provide a full 256bit value as a parameter. Now, we can execute the contract directly:
    ...
    ./yupostproject-cli sendtocontract $CONTRACT 3fb5c1cb000000000000000000000000000000000000000000000000000000000001e240
...
This will send 10 tokens to the contract:
    ...
    ./yupostproject-cli sendtocontract $CONTRACT d0e30db0 10

And then, to withdraw them it's also very simple:

    ./yupostproject-cli sendtocontract $CONTRACT 3ccfd60b
...
If you want to control the exact address that the contract sends coins to, you can also explicitly specify the sender. Note that you must be capable of sending coins from that address (you can't use someone elses address). One of my wallet addresses is qZbjaE8N18ZU1m7851G7QGhvxKL74SRBTt, so I'll use that:
    ...
    ./yupostproject-cli sendtocontract $CONTRACT 3ccfd60b 0 190000 0.0000001 qZbjaE8N18ZU1m7851G7QGhvxKL74SRBTt
...
Note that if you get the error "Sender address does not have any unspent outputs", then you should send some coins to that address (they must be spent in order to prove that you own that address). This can be accomplished with any amount of coins:
    ...
    ./yupostproject-cli sendtoaddress qZbjaE8N18ZU1m7851G7QGhvxKL74SRBTt 0.001
...
* Q: "I think I found a bug in YuPost" A: Please report any bugs at https://github.com/yupostproject/yupostproject/issues
...
* `-record-log-opcodes` - This will create a new log file in the YuPost data directory (usually ~/.yupostproject) named vmExecLogs.json, where any EVM LOG opcode is logged along with topics and data that the contract requested be logged. 
...
Additional documents for the overall design and expected results of various operations is available at the ITD repository here: https://github.com/yupostproject/yupostproject-itds
